...
interface PreviewFeedbackWidgetProps {
  template: string;
  theme: string;
  ratingType: string;
  color: string;
...
const PreviewFeedbackWidget = ({ 
  template, 
  theme, 
  ratingType, 
  color,
  templateName,
}: PreviewFeedbackWidgetProps) => {
  ...
  // Reset submitted state when template changes
  useEffect(() => {
    setSubmitted(false);
    setRating(null);
    setFeedback("");
  }, [template, theme, ratingType]);
  ...
  // Render rating options based on selected rating type
  ...
  return (
    ...
    >
      ...
      <div className={getTemplateStyle()}>
        {submitted ? (
...
export default PreviewFeedbackWidget;